    ...
    <div className="fixed top-0 right-0 z-[9999] bg-black/90 text-white text-xs p-4 m-2 rounded-lg shadow-xl max-w-xs font-mono border border-white/10">
      <div className="space-y-2">
        ...
        <div className="border-t border-white/10 pt-2">
          ...
          <div><span className="text-gray-400">Account:</span> {profile?.email || 'NONE'}</div>
          <div><span className="text-gray-400">Role:</span> {profile?.role || 'PUBLIC'}</div>
        </div>
        {profile && (
          <div className="border-t border-white/10 pt-2 space-y-1">
            <button
              ...
              className="bg-red-900/50 text-red-200 px-2 py-1 rounded w-full hover:bg-red-900"
            >
              Force Sign Out
            </button>
          </div>
